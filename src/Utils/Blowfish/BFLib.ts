// From: https://github.com/egoroof/blowfish/blob/master/src/Blowfish.mjs

import { MODE, PADDING, TYPE } from './BFConstants.js';
import {
  isStringOrBuffer,
  includes,
  expandKey,
...
export class Blowfish {
  ...
  constructor(key: string | Uint8Array, mode?: MODE, padding?: PADDING) {
    ...
    if (key.length < 1 || (key as Uint8Array).byteLength < 1) 
  ...
  /** Sets the IV. */
  public setIv(iv: string | Uint8Array) {
  ...
  public decode(data: string | Uint8Array, returnType = TYPE.STRING) {
    if (!isStringOrBuffer(data)) {
      throw new Error(
  ...
  private _encryptBlock(l: number, r: number) {
    for (let i = 0; i < 16; i++) {
      l = xor(l, this.p[i]);
      r = xor(r, this._f(l));
      [l, r] = [r, l];
  ...
  private _decryptBlock(l: number, r: number) {
    for (let i = 17; i > 1; i--) {
      l = xor(l, this.p[i]);
      r = xor(r, this._f(l));
      [l, r] = [r, l];
  ...
  private _encodeECB(bytes: Uint8Array | string) {
  ...
  private _encodeCBC(bytes: Uint8Array | string) {
    const encoded = new Uint8Array(bytes.length);
  ...
  private _decodeECB(bytes: Uint8Array) {
  ...
  private _decodeCBC(bytes: Uint8Array) {
    const decoded = new Uint8Array(bytes.length);
    let prevL = packFourBytes(this.iv[0], this.iv[1], this.iv[2], this.iv[3]);
    let prevR = packFourBytes(this.iv[4], this.iv[5], this.iv[6], this.iv[7]);
    let prevLTmp: number;
